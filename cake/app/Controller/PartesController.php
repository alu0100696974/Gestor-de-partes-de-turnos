...
class PartesController extends AppController {
   ...
   //Componentes
    ...
    public $uses = array('Parte','Valoresdefecto','Categoria','TipocamposTipoparte','User','Tipofamilia','Tipocampo','Reale','Entero','Texto', 'Incidencia');
	...
	**/
	...
	public function nuevoparte(){
	  ...
	  $data= array('id' => $newParteId,'tipoparte_id'=> $tipoparteid,'usuariovendedor'=>$usuario,'incidencia_id'=>'46');
	...
	public function actualizar ($id){
	  ...
	  $data1= array('id' => $id,'modificado'=>'1');
	  $this->Parte->save($data1);
		if (!$this->Parte->exists($id)) {
	 ...
	 * */
	public function validar ($id = null, $mod= null, $vend=null){
	  $this->Parte->id=$id;
	  ...
	  if($mod=='1'){
	  $this->Parte->Incidencia->incidenciaDefecto($id, $usuariogestor, $vend);
	  } 
