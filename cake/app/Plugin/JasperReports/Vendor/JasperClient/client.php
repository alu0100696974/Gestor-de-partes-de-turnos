...
define("PROP_REFERENCE_URI", "PROP_REFERENCE_URI");
...
define('JASPER_USER', 'jasperadmin');
define('JASPER_PASS', 'jasperadmin');

class JasperWSClient {

    private $jasperserver_url;
    private $webservices_uri;
    private $namespace;

    function __construct() {
        $_SESSION["username"] = JASPER_USER;
        $_SESSION["password"] = JASPER_USER;
        $this->jasperserver_url = 'http://localhost:8081/jasperserver/services/';
...
// ws_checkUsername try to list a void URL. If no WS error occurs, the credentials are fine
    function ws_checkUsername($username, $password) {
        ...
        $response = $info->call("list", $params, array('namespace' => NAME));
    ...
    function ws_list($uri, $args = array()) {
        ...
        $connection_params = array("user" => JASPER_USER, "pass" => JASPER_PASS);
        $info = new SOAP_client($this->webservices_uri, false, false, $connection_params);
    ...
    function ws_runReport($uri, $report_params, $output_params, &$attachments) {
        ...
        $connection_params = array("user" => JASPER_USER, "pass" => JASPER_PASS);
        $connection_params["timeout"] = $max_execution_time;
        $info = new SOAP_client($this->webservices_uri, false, false, $connection_params);
...
// Sample to put something on the server
    function ws_put() {
        ...
        $info = new SOAP_client($this->webserices_uri, false, false, $connection_params);
